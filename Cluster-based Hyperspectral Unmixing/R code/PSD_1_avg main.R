...
average_diss = PSD_1_avg(data, gridSizeX, gridSizeY)
...
plot(ngroups_first, theta, type = 'l', col = "black", xaxt = 'n', ylab = expression(theta), 
     xlab = "Subgroups", main = "Theta values after single k-means - PSD[1] AVG", ylim = c(0,1))
...
matplot(k_values, theta_matrix, type = 'p', xaxt = 'n', pch = 19, cex = 1.2, ylab = expression(theta), 
        col = colors, main = "PSD[1] AVG - Theta values", ylim = c(0,1))
...
plot_grid(gridSizeX, gridSizeY, final_cluster, 
          title = paste("PSD[1] AVG -", k, "clusters and", ngroups, "subgroups"))
