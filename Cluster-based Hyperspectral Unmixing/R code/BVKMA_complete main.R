...
matplot(n, eta_matrix, pch = 19, type = 'p', xaxt = 'n', xlab = "n", ylab = "Normalized Entropy",
        col = colors, cex = 1.2, main = "BVKMA - Normalized Entropy", ylim = c(0,1))
...
matplot(k, t(theta_matrix[c("100", "150", "200", "250"),]), type = 'p', pch = 19, xlab = "Number of clusters", ylab = expression(theta), cex = 1.2, 
        col = colors, main = "BVKMA - Theta values", ylim = c(0,1))
matlines(k, t(theta_matrix[c("100", "150", "200", "250"),]), lty = 1, lwd = 2, col = colors)
legend("topleft", legend = c("n = 100", "n = 150", "n = 200", "n = 250"), 
...
cluster = BVKMA(data, gridSizeX, gridSizeY, n, k, B, print = FALSE)$cluster

plot_grid(gridSizeX, gridSizeY, cluster, title = paste("CLR + BVKMA -", k, "clusters and n =", n),
          scaled = 0, centroids = TRUE, data = data, 
          title_centr = paste("Centroids when k =", k, "and n =", n), 
          original = TRUE, original_data = original_data)

save(cluster, file = "clusterization_complete_150_4.RData")
