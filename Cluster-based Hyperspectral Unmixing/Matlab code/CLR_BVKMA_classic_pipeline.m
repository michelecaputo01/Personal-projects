...
for cl = 1:k
    ...
    Elems_per_row = ceil(sqrt(size(classic_endm,2)));

    figure

    for i = 1:size(classic_endm,2)
        m = match(match(:,2)== i,1);
        if isempty(m)
            continue
        end
        ...
        title(['Endmember ', num2str(i), ' in cluster ', num2str(cl), ' (classic)'])
    ...
    check_ANC_violations(f_3D, ['FCLS (cluster ', num2str(cl) ') - ANC violations (classic) - Endm'], used_colormap);

    check_ASC_violations(f_3D, ['FCLS (cluster ', num2str(cl) ') - ASC violations (classic)'], used_colormap);
    title(['FCLS (cluster ', num2str(cl) ') - ASC violations (classic)'])

    check_abundances_sum(f_3D, ['FCLS (cluster ', num2str(cl) ') - Abundances sum (classic)'], used_colormap);
    title(['FCLS (cluster ', num2str(cl) ') - Abundances sum (classic)'])
    ...
    plot_total_abundances(f_matched_3D, ['FCLS (cluster ', num2str(cl) ') - Total abundances after endmembers matching (classic)']);
    hold on
    ...
    number_active_endmembers(f_3D, ['FCLS (cluster ', num2str(cl) ') - Total active endmembers (classic)'], used_colormap);
    clim([0 16])
    title(['FCLS (cluster ', num2str(cl) ') - Total active endmembers (classic)'])
    ...
    check_ANC_violations(c_3D, ['CLS (cluster ', num2str(cl) ') - ANC violations (classic) - Endm'], used_colormap);

    check_ASC_violations(c_3D, ['CLS (cluster ', num2str(cl) ') - ASC violations (classic)'], used_colormap);
    title(['FCLS (cluster ', num2str(cl) ') - ASC violations (classic)'])

    check_abundances_sum(c_3D, ['CLS (cluster ', num2str(cl) ') - Abundances sum (classic)'], used_colormap);
    title(['CLS (cluster ', num2str(cl) ') - Abundances sum (classic)'])
    ...
    N_pixels = length(indexes);
    plot_total_abundances(c_matched_3D, ['CLS (cluster ', num2str(cl) ') - Total abundances after endmembers matching (classic)']);
    hold on
    yline(N_pixels/N_endmembers)
    hold off
    ...
    number_active_endmembers(c_3D, ['CLS (cluster ', num2str(cl) ') - Total active endmembers (classic)'], used_colormap);
    clim([0 16])
    title(['CLS (cluster ', num2str(cl) ') - Total active endmembers (classic)'])
    ...
    title(['FCLS total abundances (classic) - Cluster ', num2str(cl)])
  
    subplot(1,2,2)
    bar(sum(c_matched'), 'red')
    title(['CLS total abundances (classic) - Cluster ', num2str(cl)])
    ...
    colorbar
    title(['Dissimilarity values between the endmembers (classic) and the library spectra in cluster ', num2str(cl)])
    ...
    for i = 1:n_endmembers
        ...
        xlabel('Wavelength')
        ylabel('Reflectance')
        title(strcat(['Cluster ', num2str(cl),' (classic) - Endmember ', num2str(real_index)]))
        xlim([min(wavlen), max(wavlen)])
        ylim([0 1])
...
title('Purest pixels for each cluster (classic)')
hold on
...
number_active_endmembers(a_FCLS_3D, 'FCLS (grouped clusters) - Total active endmembers (classic)', used_colormap);
clim([0 16])
title('FCLS (grouped clusters) - Total active endmembers (classic)')
...
colorbar
title('FCLS - Reconstruction error per band per pixel (clustered classic)')
clim([0 0.16])
xlabel('Pixel')
ylabel('Band')
...
number_active_endmembers(a_CLS_3D, 'CLS (grouped clusters) - Total active endmembers (classic)', used_colormap);
clim([0 16])
title('CLS (grouped clusters) - Total active endmembers (classic)')
...
colorbar
title('CLS - Reconstruction error per band per pixel (clustered classic)')
clim([0 0.16])
xlabel('Pixel')
ylabel('Band')
...
plot_total_abundances(f_total_matched_3D, 'FCLS - Total abundances after endmembers matching (classic)');
hold on
...
N_pixels = Lines*Columns;
plot_total_abundances(c_total_matched_3D, 'CLS - Total abundances after endmembers matching (classic)');
hold on
yline(N_pixels/size(classic_endm,2))
hold off
...
plot_cube_3D(f_total_matched_3D, 'FCLS (Matched) (classic) - Abund.', used_colormap, [0 1]);

plot_cube_3D(c_total_matched_3D, 'CLS (Matched) (classic) - Abund.', used_colormap, [0 1]);
...
disp(['FCLS frob error (with clusters) = ', num2str(FCLS_fro_error)]); % 0.0057375
disp(['CLS frob error (with clusters) = ', num2str(CLS_fro_error)]); % 0.0039496
disp(['rmsSAE (with clusters) = ', num2str(rmsSAE)]); % 0.060463
...
if save
    ...
    save_folder = "C:\Users\miche\Desktop\Michele\PoliMi\Magistrale\TESI\Codici personali\Immagini\CLR + BVKMA + classic pipeline matlab";
