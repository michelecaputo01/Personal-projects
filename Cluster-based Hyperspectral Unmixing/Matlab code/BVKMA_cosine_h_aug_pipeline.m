...
for cl = 1:k
    ...
    for i = 1:n_endmembers
        ...
        ylabel('Reflectance')
        if isempty(real_index)
            title(strcat(['Cluster ', num2str(cl),' - Endmember (not matched)']))
        else
...
disp(['FCLS frob error (with clusters) = ', num2str(FCLS_fro_error)]); % 0.0060705
disp(['CLS frob error (with clusters) = ', num2str(CLS_fro_error)]); % 0.0045381
disp(['rmsSAE (with clusters) = ', num2str(rmsSAE)]); % 0.044473
...
if save
    ...
    save_folder = "C:\Users\miche\Desktop\Michele\PoliMi\Magistrale\TESI\Codici personali\Immagini\BVKMA cosine + h_aug pipeline matlab";
