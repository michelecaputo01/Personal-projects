...
fcns{2} = makeinterp(linspace(0,1,21));   % low-fidelity
fcns{3} = makeinterp(linspace(0.1,0.9,9));   % lowest-fidelity
...
title([method,' main sensitivity estimates for Ishigami function (with splines)'],'interpreter','latex')
...
if strcmp(method,'Owen') || strcmp(method,'Saltelli')
    ...
    bp = gca;   bp.XAxis.TickLabelInterpreter = 'latex';
    title([method,' total sensitivity estimates for Ishigami function (with splines)'],'interpreter','latex')
end
...
title('Variance estimates for Ishigami function (with splines)','interpreter','latex')