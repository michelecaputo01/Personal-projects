...
fcns{2} = makeinterp(linspace(0.1,0.9,5));   % low-fidelity
fcns{3} = makeinterp(linspace(0.25,0.75,3));   % lowest-fidelity
...
title([method,' main sensitivity estimates for Borehole function'],'interpreter','latex')

if strcmp(method,'Owen') || strcmp(method,'Saltelli')
    % plot total effect sensitivity indices
    figure(2); clf
    h = boxplot([mc_st(:,1), mf_st(:,1), mc_st(:,2), mf_st(:,2),mc_st(:,3),mf_st(:,3),mc_st(:,4),mf_st(:,4),mc_st(:,5),mf_st(:,5),...
    mc_st(:,6),mf_st(:,6),mc_st(:,7),mf_st(:,7),mc_st(:,8),mf_st(:,8)],...
        'Colors',[blue; red; blue; red; blue; red],'Whisker',10,...
        'labels',{'MC $s_t^1$','MF $s_t^1$','MC $s_t^2$','MF $s_t^2$','MC $s_t^3$','MF $s_t^3$',...
    'MC $s_t^4$','MF $s_t^4$','MC $s_t^5$','MF $s_t^5$','MC $s_t^6$','MF $s_t^6$',...
    'MC $s_t^7$','MF $s_t^7$','MC $s_t^8$','MF $s_t^8$'});
    set(h,{'linew'},{2}); grid on
    ...
    bp = gca;   bp.XAxis.TickLabelInterpreter = 'latex';
    title([method,' total sensitivity estimates for Borehole function'],'interpreter','latex')
end
...
title('Variance estimates for Borehole function','interpreter','latex')