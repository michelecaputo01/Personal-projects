...
d = 2;          % dimension of uncertain input
...
figure(1); clf
h = boxplot([mc_sm(:,1), mf_sm(:,1), mc_sm(:,2), mf_sm(:,2)],...
    'Colors',[blue; red; blue; red],'Whisker',10,...
    'labels',{'MC $s_m^1$','MF $s_m^1$','MC $s_m^2$','MF $s_m^2$'});
...
title([method,' main sensitivity estimates for McCormick function(p=4000)'],'interpreter','latex')

if strcmp(method,'Owen') || strcmp(method,'Saltelli')
    % plot total effect sensitivity indices
    figure(2); clf
    h = boxplot([mc_st(:,1), mf_st(:,1), mc_st(:,2), mf_st(:,2)],...
        'Colors',[blue; red; blue; red],'Whisker',10,...
        'labels',{'MC $s_t^1$','MF $s_t^1$','MC $s_t^2$','MF $s_t^2$'});
    set(h,{'linew'},{2}); grid on
    ...
    bp = gca;   bp.XAxis.TickLabelInterpreter = 'latex';
    title([method,' total sensitivity estimates for McCormick function(p=4000)'],'interpreter','latex')
end
...
title('Variance estimates for McCormick function(p=4000)','interpreter','latex')